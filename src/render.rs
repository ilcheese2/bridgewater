...
impl Camera {
    pub fn new() -> Self {
        Self {
            position: Vec3::new(500.0, 300.0, -800.0),
            forward: Vec3::new(0., 0., -1.0),
            perspective: Mat4::perspective_lh(PI / 4.0, 1.0, 0.1, 1000.0),
        }
    }
...
impl RenderParticles {
    pub fn new(num_particles: usize, metal_context: &MetalContext) -> Self {
        ...
        let cube_indices = [ // thanks https://stackoverflow.com/a/58775844
            [2u16, 6, 7],
            [2, 3, 7],
            [0, 4, 5],
            [0, 1, 5],
        ...
        render_pipeline_desc.set_vertex_function(Some(&metal_context.library.get_function("vertexShader", None).unwrap()));
        render_pipeline_desc.set_fragment_function(Some(&metal_context.library.get_function("fragmentShader", None).unwrap()));
        render_pipeline_desc.color_attachments().object_at(0).unwrap().set_pixel_format(MTLPixelFormat::BGRA8Unorm);
        ...
        Self {
            ...
            render_pipeline_state: metal_context.device.new_render_pipeline_state(&render_pipeline_desc).unwrap()
        }
    }


    pub fn render(&mut self, buffer: &Buffer, drawable: &MetalDrawableRef, metal_context: &MetalContext) {
        let command_buffer = self.command_queue.new_command_buffer();

        let render_pass_desc = RenderPassDescriptor::new();
        render_pass_desc.color_attachments().object_at(0).unwrap().set_load_action(MTLLoadAction::Clear);
        render_pass_desc.color_attachments().object_at(0).unwrap().set_clear_color(MTLClearColor::new(41.0/255.0, 42.0/255.0, 48.0/255.0, 1.0));
        render_pass_desc.color_attachments().object_at(0).unwrap().set_texture(Some(drawable.texture()));
        ...
        command_encoder.draw_indexed_primitives_instanced(MTLPrimitiveType::Triangle, 36, MTLIndexType::UInt16, &self.cube_indices, 0, 300);
